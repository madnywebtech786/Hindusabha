...
/**
 * Shared secondary-page opener: the cream ground, ambient motif field,
 * and staggered title reveal used by every interior page hero (About,
 * Leadership, Contact, Gallery, In the News) — content-only, so each
 * page keeps its own headline, copy, and shloka.
 */
export default function PageHero({ eyebrow, title, highlight, description, shloka }) {
  return (
    <section className="relative overflow-hidden bg-cream">
      ...
      <div className="relative mx-auto flex max-w-4xl flex-col items-center px-5 py-24 text-center sm:px-8 sm:py-32">
        ...
        >
          {title} {highlight ? <span className="text-gradient italic">{highlight}</span> : null}
        </motion.h1>
